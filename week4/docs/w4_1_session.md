...
이번 실습에서는 유형과 장르 비교까지 이어가기 위해 메타데이터와 매칭되는 콘텐츠를 분석 대상으로 둡니다.
...
장르별 비교가 필요할 때는 콘텐츠 성과 요약 뒤에
`netflix_titles`에서 만든 `title_clean + genre` 형태의 장르 bridge를 붙입니다.
이렇게 하면 성과 지표는 콘텐츠 단위로 중복 없이 계산하고, 장르 관점 비교도 유지할 수 있습니다.
...
모델 입력 후보의 기본 grain은 `week_date + title_clean`이며,
장르가 필요하면 `netflix_titles`에서 만든 별도 장르 bridge를 조인해서 확장합니다.
