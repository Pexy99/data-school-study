...
따라서 장르 long format mart에 바로 `LAG()`를 쓰기보다,
먼저 `week_date + title_clean` 단위의 콘텐츠-주차 중간 테이블을 만든 뒤 계산합니다.
...
`title_clean + genre` 형태의 장르 bridge를 붙입니다.
...
장르가 필요하면 별도 장르 bridge를 조인해서 확장합니다.
