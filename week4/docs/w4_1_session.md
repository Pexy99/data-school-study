...
- 콘텐츠-주차 단위에서 순위 변화 feature와 롱런 label을 만든다.
...
각 콘텐츠 주차별 `prev_week_rank`, `rank_change`, `is_new_entry`는 feature table에 더 가깝습니다.
...
콘텐츠의 이전 주 변화나 롱런 여부를 보려는 Q3~Q4에는
...
이번 실습에서는 유형 정보까지 붙은 콘텐츠-주차 테이블을 만든 뒤,
순위 변화와 롱런 여부를 계산합니다.
...
다만 특정 시즌에 "급상승"하는 장르까지 보려면 단순 집계만으로는 부족합니다.
계절별 장르 집계 결과를 다시 기간 순서로 놓고, 이전 기간 대비 변화량을 정의해야 합니다.
...
이전 기간 대비 등장 횟수나 평균 순위 변화로 정의하는 것입니다.

즉 Q2의 기본 선호 장르 분석은 long format mart로 풀고,
급상승 분석은 기간별 장르 집계 테이블을 한 단계 더 만든 뒤 확장하는 문제로 분리합니다.
...
여기서는 장르가 아니라 콘텐츠의 주차별 성과 흐름이 핵심입니다.
따라서 장르 long format mart에 바로 `LAG()`를 쓰거나 mart를 다시 접기보다,
먼저 raw 성과 테이블에서 `week_date + title_clean` 단위의 콘텐츠-주차 중간 테이블을 만든 뒤 계산합니다.

이 중간 테이블에서는 같은 콘텐츠-주차가 한 행만 남도록 먼저 요약합니다.
...
예를 들어 같은 콘텐츠의 이전 주 순위를 가져온 뒤,
현재 순위와 비교하면 순위 변화 feature를 만들 수 있습니다.
...
그다음 콘텐츠별로 얼마나 오래 Top10에 머물렀는지,
가장 좋은 순위가 무엇이었는지 확인할 수 있습니다.
...
- `week_row_num`: 같은 콘텐츠가 Top10에 등장한 주차 순서

이 feature들은 급상승 여부나 인기 변화 흐름을 보기 위해 필요합니다.

이번 세션에서는 이 feature들을 `week_date + title_clean` grain에서 계산합니다.
...
4. Q3~Q4는 raw 성과 테이블에서 콘텐츠-주차 intermediate를 새로 만든다.
5. 콘텐츠별 순위 변화 feature와 롱런 label을 만든다.
